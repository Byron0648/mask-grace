...
class MaskModel(torch.nn.Module):
    def __init__(self, encoder: Encoder, num_hidden: int, num_proj_hidden: int,
        ...
        self.fc1 = torch.nn.Linear(num_hidden, num_proj_hidden)
        self.fc2 = torch.nn.Linear(num_proj_hidden, num_hidden)
        self.fc1_ = torch.nn.Linear(num_hidden, num_proj_hidden)
        self.fc2_ = torch.nn.Linear(num_proj_hidden, num_hidden)

    def forward(self, x: torch.Tensor,
                edge_index: torch.Tensor,
                masks=None) -> torch.Tensor:
        y = self.encoder(x, edge_index)
    ...
    def projection_(self, z: torch.Tensor) -> torch.Tensor:
        z = F.elu(self.fc1_(z))
        return self.fc2_(z)
